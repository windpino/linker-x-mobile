...
function App() {
  ...
  const closeTopModal = () => {
    try {
      if (typeof isMobileMenuOpen !== 'undefined' && isMobileMenuOpen) { setIsMobileMenuOpen(false); return true; }
      if (typeof isCalendarModalOpen !== 'undefined' && isCalendarModalOpen) { setIsCalendarModalOpen(false); return true; }
      if (typeof isScheduleDetailOpen !== 'undefined' && isScheduleDetailOpen) { setIsScheduleDetailOpen(false); setSelectedScheduleForDetail(null); return true; }
      if (typeof isScheduleRegistrationOpen !== 'undefined' && isScheduleRegistrationOpen) { setIsScheduleRegistrationOpen(false); return true; }
      if (typeof isScheduleListOpen !== 'undefined' && isScheduleListOpen) { setIsScheduleListOpen(false); return true; }
      if (typeof isSalesInvoiceOpen !== 'undefined' && isSalesInvoiceOpen) { setIsSalesInvoiceOpen(false); return true; }
      if (typeof isSalesInvoiceListOpen !== 'undefined' && isSalesInvoiceListOpen) { setIsSalesInvoiceListOpen(false); return true; }
      if (typeof isSalesLedgerOpen !== 'undefined' && isSalesLedgerOpen) { setIsSalesLedgerOpen(false); return true; }
      if (typeof isSalesOrderOpen !== 'undefined' && isSalesOrderOpen) { setIsSalesOrderOpen(false); return true; }
      if (typeof isOrderListOpen !== 'undefined' && isOrderListOpen) { setIsOrderListOpen(false); return true; }
      if (typeof isPurchaseInvoiceOpen !== 'undefined' && isPurchaseInvoiceOpen) { setIsPurchaseInvoiceOpen(false); return true; }
      if (typeof isPurchaseLedgerOpen !== 'undefined' && isPurchaseLedgerOpen) { setIsPurchaseLedgerOpen(false); return true; }
      if (typeof isPurchaseOrderOpen !== 'undefined' && isPurchaseOrderOpen) { setIsPurchaseOrderOpen(false); return true; }
      if (typeof isInventoryTransferOpen !== 'undefined' && isInventoryTransferOpen) { setIsInventoryTransferOpen(false); return true; }
      if (typeof isInventoryMovementManagerOpen !== 'undefined' && isInventoryMovementManagerOpen) { setIsInventoryMovementManagerOpen(false); return true; }
      if (typeof isPartnerManagerOpen !== 'undefined' && isPartnerManagerOpen) { setIsPartnerManagerOpen(false); return true; }
      if (typeof isProductManagerOpen !== 'undefined' && isProductManagerOpen) { setIsProductManagerOpen(false); return true; }
      if (typeof isWarehouseManagerOpen !== 'undefined' && isWarehouseManagerOpen) { setIsWarehouseManagerOpen(false); return true; }
      if (typeof isStaffManagerOpen !== 'undefined' && isStaffManagerOpen) { setIsStaffManagerOpen(false); return true; }
      if (typeof isAccountManagerOpen !== 'undefined' && isAccountManagerOpen) { setIsAccountManagerOpen(false); return true; }
      if (typeof isCashReportOpen !== 'undefined' && isCashReportOpen) { setIsCashReportOpen(false); return true; }
      if (typeof isSalesReportOpen !== 'undefined' && isSalesReportOpen) { setIsSalesReportOpen(false); return true; }
      if (typeof isOrderReportOpen !== 'undefined' && isOrderReportOpen) { setIsOrderReportOpen(false); return true; }
      if (typeof isInventoryReportOpen !== 'undefined' && isInventoryReportOpen) { setIsInventoryReportOpen(false); return true; }
      if (typeof isReceivablesReportOpen !== 'undefined' && isReceivablesReportOpen) { setIsReceivablesReportOpen(false); return true; }
      if (typeof isEditDeleteReportOpen !== 'undefined' && isEditDeleteReportOpen) { setIsEditDeleteReportOpen(false); return true; }
      if (typeof isCashBookOpen !== 'undefined' && isCashBookOpen) { setIsCashBookOpen(false); return true; }
      if (typeof isExpenseRegistrationOpen !== 'undefined' && isExpenseRegistrationOpen) { setIsExpenseRegistrationOpen(false); return true; }
      if (typeof isStaffPerformanceReportOpen !== 'undefined' && isStaffPerformanceReportOpen) { setIsStaffPerformanceReportOpen(false); return true; }
      if (typeof isDataManagerOpen !== 'undefined' && isDataManagerOpen) { setIsDataManagerOpen(false); return true; }
      if (typeof isPartnerBulkOpen !== 'undefined' && isPartnerBulkOpen) { setIsPartnerBulkOpen(false); return true; }
      if (typeof isProductBulkOpen !== 'undefined' && isProductBulkOpen) { setIsProductBulkOpen(false); return true; }
      if (typeof isPartnerExcelOpen !== 'undefined' && isPartnerExcelOpen) { setIsPartnerExcelOpen(false); return true; }
      if (typeof isProductExcelOpen !== 'undefined' && isProductExcelOpen) { setIsProductExcelOpen(false); return true; }
      if (typeof isPurchaseLedgerExcelOpen !== 'undefined' && isPurchaseLedgerExcelOpen) { setIsPurchaseLedgerExcelOpen(false); return true; }
      if (typeof isSalesLedgerExcelOpen !== 'undefined' && isSalesLedgerExcelOpen) { setIsSalesLedgerExcelOpen(false); return true; }
      if (typeof isSettingsOpen !== 'undefined' && isSettingsOpen) { setIsSettingsOpen(false); return true; }
      if (typeof isLicenseOpen !== 'undefined' && isLicenseOpen) { setIsLicenseOpen(false); return true; }
      if (typeof isDashboardSettingsOpen !== 'undefined' && isDashboardSettingsOpen) { setIsDashboardSettingsOpen(false); return true; }
      if (typeof isFavoriteSettingsOpen !== 'undefined' && isFavoriteSettingsOpen) { setIsFavoriteSettingsOpen(false); return true; }
      if (typeof selectedSystemNotice !== 'undefined' && selectedSystemNotice) { setSelectedSystemNotice(null); return true; }
      if (typeof isInventoryAdjustmentOpen !== 'undefined' && isInventoryAdjustmentOpen) { setIsInventoryAdjustmentOpen(false); return true; }
      if (typeof isInventoryMismatchOpen !== 'undefined' && isInventoryMismatchOpen) { setIsInventoryMismatchOpen(false); return true; }
      if (typeof isTaxReportOpen !== 'undefined' && isTaxReportOpen) { setIsTaxReportOpen(false); return true; }
      if (typeof printingTaxInvoice !== 'undefined' && printingTaxInvoice) { setPrintingTaxInvoice(null); return true; }
      if (typeof isRecentActivityModalOpen !== 'undefined' && isRecentActivityModalOpen) { setIsRecentActivityModalOpen(false); return true; }
      if (typeof orderingPartner !== 'undefined' && orderingPartner) { setOrderingPartner(null); return true; }
      return false;
    } catch (e) {
      return false;
    }
  ...
  // 모바일 뒤로가기(popstate) 안전 연동
  const closeTopModalRef = React.useRef(closeTopModal);
  closeTopModalRef.current = closeTopModal;

  React.useEffect(() => {
    try {
      window.history.pushState({ page: 'linker_app' }, '', window.location.href);
    } catch (e) {}
  ...
  React.useEffect(() => {
    ...
    const cleanupLogs = async () => {
      try {
        ...
        const targetCollections = ['actionLogs'];
  ...
  const onMoveStock = async (from, to, productName, qty, isAuto = false, customDate = null, relatedOrderId = null) => {
    try {
      ...
      const nextHistory = [newEntry, ...inventoryTransferHistory];
      setInventoryTransferHistory(nextHistory);
      await saveBundle(companyId, 'inventoryTransferHistory', nextHistory);
  ...
  const onDeleteMoveStock = async (historyIdOrIds) => {
    try {
      ...
      await batch.commit();
      const nextHistory = inventoryTransferHistory.filter(h => !ids.includes(String(h.id)));
      setInventoryTransferHistory(nextHistory);
      await saveBundle(companyId, 'inventoryTransferHistory', nextHistory);
  ...
  const onUpdateInventoryTransfer = async (id, updatedEntry) => {
    try {
      const companyId = currentUser?.companyId || 'default';
      const cleanEntry = cleanForFirestore({ ...updatedEntry, updatedAt: new Date().toISOString() });
      await setDoc(doc(db, 'companies', companyId, 'inventoryTransferHistory', String(id)), cleanEntry);
      const nextTransfers = inventoryTransferHistory.map(h => String(h.id) === String(id) ? { ...h, ...cleanEntry } : h);
      setInventoryTransferHistory(nextTransfers);
      await saveBundle(companyId, 'inventoryTransferHistory', nextTransfers);
    } catch (err) {
      console.error('Error updating inventory transfer history:', err);
    }
  };
  ...
  const onDeleteAllTransfers = async () => {
    try {
      ...
      await saveBundle(companyId, 'inventoryTransferHistory', []);
      ...
      await logOperation({
        ...
        type: '삭제',
        title: '재고이동 전체 삭제',
        detail: `총 ${totalCount}건의 재고 이동 이력이 일괄 삭제되었습니다. (재고 수량은 현 상태를 유지합니다)`,
        user: currentUser?.name || '시스템'
      });
    } catch (err) {
      console.error('Error deleting all transfers:', err);
    }
  };
  ...
  const onRevertAutoMoveStock = async (from, to, productName, qty, relatedOrderId = null) => {
    try {
      ...
      const match = [...inventoryTransferHistory]
        .reverse()
        .find(h => 
          (relatedOrderId ? String(h.salesOrderId) === String(relatedOrderId) : (h.from === from && h.to === to)) &&
          h.item === productName && 
      ...
      if (match) {
        ...
        const nextHistory = inventoryTransferHistory.filter(h => String(h.id) !== String(match.id));
        setInventoryTransferHistory(nextHistory);
        await saveBundle(companyId, 'inventoryTransferHistory', nextHistory);
      }
    } catch (err) {
  ...
  const handleDeleteSalesInvoice = async (id) => {
    ...
    if (!checkWritePermission(targetInv?.creator)) {
      alert('수정/삭제 권한이 없습니다 (본인이 작성한 전표만 삭제 가능).');
      return;
    }
    try {
      const companyId = currentUser?.companyId || 'default';
      ...
      const wh = targetInv?.warehouse || userWarehouse;
      if (wh && targetInv?.items && targetInv.items.length > 0) {
        if (!nextInv[wh]) nextInv[wh] = {};
        for (const item of targetInv.items) {
          nextInv[wh][item.name] = (nextInv[wh][item.name] || 0) + Number(item.qty);
        }
      }
      ...
      const nextTransfers = inventoryTransferHistory.filter(h => String(h.salesInvoiceId) !== String(id));
      setInventoryTransferHistory(nextTransfers);
      await saveBundle(companyId, 'inventoryTransferHistory', nextTransfers);
      ...
      if (linkedOrders.length > 0) {
        ...
        const updatedCache = curCache.map(so => {
          if (linkedOrders.some(lo => String(lo.id) === String(so.id))) {
            return { ...so, status: '대기', isCompleted: false, salesInvoiceId: null };
          }
          return so;
        });
      ...
      await logOperation({
        ...
        detail: `출고 재고 원상 복구 및 전표 삭제 (전표일자: ${targetInv?.date || '-'}, 품목수: ${targetInv?.items?.length || 0}건, 합계: ${(targetInv?.totalAmount || 0).toLocaleString()}원)`,
        user: currentUser?.name || '시스템',
        targetId: String(id)
      });
  ...
  const handleDeleteSalesOrder = async (id) => {
    ...
    try {
      ...
      // 1. 주문서 ID(salesOrderId / orderId) 또는 연결된 매출전표(salesInvoiceId)에 매핑된 모든 이동 내역 탐색
      const idMatches = inventoryTransferHistory.filter(h => 
        String(h.salesOrderId) === String(id) || 
        String(h.orderId) === String(id) ||
        (targetOrder?.salesInvoiceId && String(h.salesInvoiceId) === String(targetOrder.salesInvoiceId))
      );

      for (const match of idMatches) {
        const { from, to, item, qty } = match;
        const numQty = Number(qty) || 0;
        if (from && to && item && numQty > 0) {
          if (!nextInv[from]) nextInv[from] = { ...(inventory[from] || {}) };
          if (!nextInv[to]) nextInv[to] = { ...(inventory[to] || {}) };
          
          if (to === '매출출고') {
            nextInv[from][item] = (nextInv[from][item] || 0) + numQty;
          } else if (from === '매입입고') {
            nextInv[to][item] = (nextInv[to][item] || 0) - numQty;
          } else {
            nextInv[from][item] = (nextInv[from][item] || 0) + numQty;
            nextInv[to][item] = (nextInv[to][item] || 0) - numQty;
          }

          revertedMovements.push({ from, to, item, qty: numQty });
        }
        await deleteDoc(doc(db, 'companies', companyId, 'inventoryTransferHistory', String(match.id)));
        deletedIds.add(String(match.id));
      }

      // 2. targetOrder의 품목 목록(items 또는 itemsText 파싱) 기반 상차/창고이동 내역 역복구 및 삭제
      if (targetOrder) {
        ...
        if (Array.isArray(targetOrder.items) && targetOrder.items.length > 0) {
          orderItems = targetOrder.items;
        } else if (typeof targetOrder.itemsText === 'string' && targetOrder.itemsText.trim()) {
          const tokens = targetOrder.itemsText.trim().split(/\s+/);
          orderItems = tokens.map(token => {
            const match = token.match(/^([^\d]+)(\d+)?$/);
            if (match) {
              const namePart = match[1].trim();
              const qtyPart = parseInt(match[2] || '1', 10);
              const found = products.find(p => p.name === namePart || p.abbreviation === namePart);
              return {
                name: found ? found.name : namePart,
                qty: qtyPart,
                loaded: true
              };
            }
        ...
        for (const item of orderItems) {
          ...
          if (itemQty > 0 && (item.loaded || matchedCount === 0)) {
            const unlinkedMatches = [...inventoryTransferHistory]
              .filter(h => 
                !deletedIds.has(String(h.id)) &&
                h.date === targetOrder.date &&
                h.item === item.name &&
                (
                  (h.from === itemFromWH && h.to === itemToWH) ||
                  (h.memo === '상차(자동이동)' || (typeof h.memo === 'string' && (h.memo.includes('상차') || h.memo.includes(String(id)) || (targetOrder.partner && h.memo.includes(targetOrder.partner)))))
                )
              );

            for (const unlinkedMatch of unlinkedMatches) {
              const uQty = Number(unlinkedMatch.qty) || 0;
              const uFrom = unlinkedMatch.from;
              const uTo = unlinkedMatch.to;

              if (uFrom && uTo && item.name && uQty > 0) {
                if (!nextInv[uFrom]) nextInv[uFrom] = { ...(inventory[uFrom] || {}) };
                if (!nextInv[uTo]) nextInv[uTo] = { ...(inventory[uTo] || {}) };
                if (uTo === '매출출고') {
                  nextInv[uFrom][item.name] = (nextInv[uFrom][item.name] || 0) + uQty;
                } else if (uFrom === '매입입고') {
                  nextInv[uTo][item.name] = (nextInv[uTo][item.name] || 0) - uQty;
                } else {
                  nextInv[uFrom][item.name] = (nextInv[uFrom][item.name] || 0) + uQty;
                  nextInv[uTo][item.name] = (nextInv[uTo][item.name] || 0) - uQty;
                }
                revertedMovements.push({ from: uFrom, to: uTo, item: item.name, qty: uQty });
              }
      ...
      const curCache = JSON.parse(localStorage.getItem('salesOrders')) || [];
      const updatedCache = curCache.filter(so => String(so.id) !== String(id));
      localStorage.setItem('salesOrders', JSON.stringify(updatedCache));
      localStorage.setItem(`salesOrders_${companyId}`, JSON.stringify(updatedCache));
      localStorage.setItem(`bundle_salesOrders_${companyId}`, JSON.stringify(updatedCache));
      ...
      if (deletedIds.size > 0) {
        ...
        const curTransferCache = JSON.parse(localStorage.getItem('inventoryTransferHistory')) || [];
        const updatedTransferCache = curTransferCache.filter(h => !deletedIds.has(String(h.id)));
        localStorage.setItem('inventoryTransferHistory', JSON.stringify(updatedTransferCache));
        localStorage.setItem(`inventoryTransferHistory_${companyId}`, JSON.stringify(updatedTransferCache));
        localStorage.setItem(`bundle_inventoryTransferHistory_${companyId}`, JSON.stringify(updatedTransferCache));
      }

      // 6. 감사 로그 기록
      const orderItemsSummary = (targetOrder?.items || []).map(i => `${i.name}(${i.qty}개)`).join(', ') || '-';
      const revertSummary = revertedMovements.length > 0 
        ? ` (상차 재고 복구 및 이동내역 ${revertedMovements.length}건 삭제: ${revertedMovements.map(m => `${m.item} ${m.qty}개 [${m.to} ➔ ${m.from}]`).join(', ')})`
        : '';
      ...
      await logOperation({
        ...
        detail: `수주일자: ${targetOrder?.date || '-'} | 품목: ${orderItemsSummary} | 합계: ${(targetOrder?.totalPrice || targetOrder?.totalAmount || 0).toLocaleString()}원 | 상차 재고 원상 복구 및 창고이동 내역 삭제 완료${revertSummary}`,
        user: currentUser?.name || '시스템',
        targetId: String(id)
      });

      alert('수주서가 삭제되었으며 상차 및 창고 이동 내역도 함께 삭제되고 재고가 원상 복구되었습니다.');
    } catch (err) {
  ...
  return (
    <div className="app-container">
      ...
      {isOrderListOpen && <OrderList 
        ...
        onEditOrder={(order) => { setEditingOrder(order); setIsSalesOrderOpen(true); setIsOrderListOpen(false); }}
        onDeleteOrder={handleDeleteSalesOrder}
        onTransferToInvoice={(invData) => { setIsOrderListOpen(false); openSalesInvoice(invData); }}

...
const SalesLedgerExcelManager = ({ onClose, salesInvoices = [], setSalesInvoices }) => {
  ...
  const MAPPING_LABELS = [
    ...
    { key: 'partner', label: '거래처명', required: true },
    ...
    { key: 'payment', label: '입금액', required: false },
  ...
  const columns = [
    ...
    { field: 'receivedAmount', header: '입금액', width: '120px', type: 'number' },
  ...
  // Step 1: Load file and show headers
    ...
    reader.onload = (evt) => {
      try {
        ...
        setMapping({
          ...
          partner: guess(['거래처', '상호', '라쿠도', 'partner']),
          ...
          payment: guess(['입금', '합금', '결제', '알인+입금', '입금+알인']),
          manager: guess(['담당자', '담 당', 'manager', '성확당']),
        });
        setStep('mapping');
      } catch (err) {
  ...
  // Step 2: Build invoices from mapping
  const handleBuildPreview = () => {
    ...
    for (let i = 0; i < rawRows.length; i++) {
      ...
      // Payment row: no product, has payment
      ...
      // Merge with previous invoice if same date+partner
      const last = invoices[invoices.length - 1];
      if (last && last.date === currentDate && last.partner === currentPartner) {
        last.items.push(item);
        last.totalAmount += lineTotal;
      } else {
        invoices.push({ id: Date.now() + i, date: currentDate, partner: currentPartner, manager: currentManager, warehouse: '', items: [item], totalAmount: lineTotal, receivedAmount: 0 });
      }
    }
    ...
    payments.forEach(pay => {
      ...
      if (targets.length > 0) targets[targets.length - 1].receivedAmount += pay.amount;
    });
  ...
  const handleConfirmImport = () => {
    ...
    const existingKeys = new Set(salesInvoices.map(inv => `${inv.date}_${inv.partner}_${inv.totalAmount}`));
    const newOnes = invoices.filter(inv => !existingKeys.has(`${inv.date}_${inv.partner}_${inv.totalAmount}`));
    setSalesInvoices([...salesInvoices, ...newOnes]);
    alert(`✅ 매출전표 ${newOnes.length}건, 입금 ${payments.length}건 등록 완료!`);
    setStep('main');
    setPreviewData({ invoices: [], payments: [] });
  };

  const handleDownload = () => {
    const dataToExport = salesInvoices.map(inv => {
    ...
    exportToExcel(dataToExport, '매출원장');
  };

  if (isBulkOpen) {
    const ed = salesInvoices.map(inv => {
      const total = inv.totalAmount || 0;
      let supply = 0;
      let tax = 0;
      if (inv.items && inv.items.length > 0) {
      ...
      <BulkEditor 
        ...
        onSave={(newData) => { 
          ...
          const mergedInvoices = newData.map(gridRow => {
            const original = salesInvoices.find(inv => inv.id === gridRow.id);
            if (original) {
              // Only update fields that are in the grid
              return {
                ...original,
                date: gridRow.date,
                ...
                receivedAmount: Number(gridRow.receivedAmount) || 0,
                totalAmount: Number(gridRow.totalAmount) || 0,
                // If itemsText was changed, we might want to flag it or handle it, 
                // but for now, we preserve the original items structure.
              };
            }
            return gridRow; // New rows added in grid (might lack full structure)
          });
          setSalesInvoices(mergedInvoices); 
        }} 
  ...
  if (step === 'mapping') {
    ...
    return (
      <WindowModal title="엑셀 컬럼 매핑" onClose={() => setStep('main')} width="850px">
        <div style={{ padding: '16px 20px' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px', marginBottom: '20px' }}>
            {MAPPING_LABELS.map(({ key, label, required }) => (
              <div key={key} style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
                <label style={{ fontSize: '0.82rem', fontWeight: 600, color: required ? '#1e293b' : '#64748b' }}>
                  {label} {required && <span style={{ color: '#ef4444' }}>*</span>}
                </label>
                <select
                  value={mapping[key]}
                  onChange={e => setMapping(prev => ({ ...prev, [key]: e.target.value }))}
                  style={{ padding: '7px 10px', borderRadius: '6px', border: `1px solid ${mapping[key] !== '' ? '#3b82f6' : '#e2e8f0'}`, fontSize: '0.85rem', background: mapping[key] !== '' ? '#eff6ff' : '#fff' }}
                >
                  <option value="">(사용안함)</option>
                  {rawHeaders.map((h, i) => (
                    <option key={i} value={String(i)}>
                      [{i}] {h} {previewRow[i] !== undefined ? `→ 예: ${String(previewRow[i]).slice(0, 15)}` : ''}
                    </option>
                  ))}
                </select>
              </div>
          ...
          <div style={{ display: 'flex', gap: '12px', justifyContent: 'flex-end', borderTop: '1px solid #f1f5f9', paddingTop: '14px' }}>
            <button onClick={() => setStep('main')} style={{ padding: '9px 20px', borderRadius: '8px', border: '1px solid #e2e8f0', background: '#f8fafc', color: '#64748b', fontWeight: 600, cursor: 'pointer' }}>
              ← 돌아가기
            </button>
            <button
              onClick={handleBuildPreview}
              disabled={!mapping.date || !mapping.partner}
              style={{ padding: '9px 24px', borderRadius: '8px', border: 'none', background: mapping.date && mapping.partner ? '#3b82f6' : '#94a3b8', color: '#fff', fontWeight: 700, cursor: mapping.date && mapping.partner ? 'pointer' : 'not-allowed', display: 'flex', alignItems: 'center', gap: '6px' }}
            >
              분석 실행 <ChevronRight size={16} />
            </button>
          </div>
        </div>
      </WindowModal>
  ...
  if (step === 'preview') {
    const { invoices, payments } = previewData;
    return (
      <WindowModal title="분석 결과 확인 및 등록" onClose={() => setStep('mapping')} width="1100px">
        <div style={{ padding: '16px 20px' }}>
          <div style={{ display: 'flex', gap: '16px', marginBottom: '14px' }}>
            <div style={{ flex: 1, background: '#eff6ff', borderRadius: '8px', padding: '12px 16px' }}>
              <div style={{ fontSize: '0.8rem', color: '#3b82f6', fontWeight: 600 }}>감지된 매출전표</div>
            ...
            <div style={{ flex: 1, background: '#f0fdf4', borderRadius: '8px', padding: '12px 16px' }}>
              <div style={{ fontSize: '0.8rem', color: '#10b981', fontWeight: 600 }}>감지된 입금내역</div>
              <div style={{ fontSize: '1.6rem', fontWeight: 700, color: '#1e293b' }}>{payments.length}건</div>
            </div>
            <div style={{ flex: 1, background: '#fefce8', borderRadius: '8px', padding: '12px 16px' }}>
              <div style={{ fontSize: '0.8rem', color: '#ca8a04', fontWeight: 600 }}>총 매출합계</div>
              <div style={{ fontSize: '1.3rem', fontWeight: 700, color: '#1e293b' }}>{invoices.reduce((s, i) => s + i.totalAmount, 0).toLocaleString()}원</div>
            </div>
          </div>
          ...
          {invoices.length === 0 ? (
            <div style={{ textAlign: 'center', padding: '40px', color: '#94a3b8', background: '#f8fafc', borderRadius: '8px' }}>
              ...
              <div style={{ fontWeight: 600 }}>데이터를 감지하지 못했습니다.</div>
              <div style={{ fontSize: '0.85rem', marginTop: '6px' }}>컬럼 매핑을 다시 확인해 주세요.</div>
              <button onClick={() => setStep('mapping')} style={{ marginTop: '16px', padding: '8px 20px', borderRadius: '8px', border: '1px solid #e2e8f0', background: '#fff', cursor: 'pointer', fontWeight: 600 }}>← 컬럼 매핑으로 돌아가기</button>
            </div>
          ) : (
              ...
              <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.82rem' }}>
                <thead>
                  <tr style={{ background: '#f8fafc', position: 'sticky', top: 0 }}>
                    {['날짜', '거래처', '품목수', '합계', '입금액', '미수금', '담당자'].map(h => (
                      <th key={h} style={{ padding: '8px 10px', textAlign: 'left', borderBottom: '1px solid #e2e8f0', color: '#64748b', fontWeight: 600 }}>{h}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                  {invoices.map((inv, idx) => (
                    <tr key={idx} style={{ borderBottom: '1px solid #f1f5f9', background: idx % 2 === 0 ? '#fff' : '#fafafa' }}>
                      ...
                      <td style={{ padding: '7px 10px', color: '#10b981' }}>{inv.receivedAmount > 0 ? `${inv.receivedAmount.toLocaleString()}원` : '-'}</td>
                      <td style={{ padding: '7px 10px', color: '#ef4444' }}>{(inv.totalAmount - inv.receivedAmount).toLocaleString()}원</td>
          ...
          <div style={{ display: 'flex', gap: '12px', justifyContent: 'flex-end', borderTop: '1px solid #f1f5f9', paddingTop: '14px' }}>
            <button onClick={() => setStep('mapping')} style={{ padding: '9px 20px', borderRadius: '8px', border: '1px solid #e2e8f0', background: '#f8fafc', color: '#64748b', fontWeight: 600, cursor: 'pointer' }}>
              ← 다시 매핑
            </button>
            {invoices.length > 0 && (
              <button onClick={handleConfirmImport} style={{ padding: '9px 24px', borderRadius: '8px', border: 'none', background: '#10b981', color: '#fff', fontWeight: 700, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '6px' }}>
                <CheckCircle size={16} /> {invoices.length}건 전표 등록 + 입금처리
              </button>
            )}
          </div>
        </div>
  ...
  // MAIN screen
  return (
    <WindowModal title="매출처원장 저장/불러오기" onClose={onClose} width="1000px">
      <div className="excel-header">
        <div className="excel-title-group">
          <FileSpreadsheet size={24} color="#3b82f6" />
          <h2 className="excel-title">매출처원장 저장/불러오기</h2>
        </div>
        <p className="excel-desc">매출 내역을 엑셀(.xlsx) 파일로 내보내거나, 외부 엑셀 파일을 업로드해 자동 분석 후 전표 및 입금처리를 일괄 등록합니다.</p>
      </div>

      <div className="excel-actions-grid">
        <div className="excel-card">
          <div className="card-top"><Download size={24} color="#3b82f6" /><span>엑셀 파일로 저장</span></div>
          <p>등록된 매출 내역(매출 일자, 거래처, 공급가, 품목 적요 등)을 엑셀 파일(.xlsx)로 내려받아 외부 정산용으로 사용합니다.</p>
          <button className="btn-excel-primary blue" onClick={handleDownload}>매출원장 다운로드 (.xlsx)</button>
        </div>

        <div className="excel-card">
          <div className="card-top"><Edit3 size={24} color="#10b981" /><span>일괄 편집 (그리드)</span></div>
          <p>일자별 매출 전표의 핵심 내용(수량, 단가, 적요 등)을 웹 그리드를 통해 파일 없이 즉각 일괄 수정하고 입력합니다.</p>
        ...
        <div className="excel-card" style={{ borderColor: '#f59e0b', background: 'linear-gradient(135deg,#fffbeb,#fef3c7)' }}>
          ...
          <p style={{ color: '#78350f', marginBottom: '12px' }}>
            외부 세무 프로그램이나 엑셀 장부의 매출 기록을 분석 및 매핑하여, 신규 전표 발행과 매출원장 자동 연동을 진행합니다.
          </p>
          <input ref={fileInputRef} type="file" accept=".xlsx,.xls,.csv" style={{ display: 'none' }} onChange={handleFileUpload} />
          <button
            className="btn-excel-primary orange"
      ...
      <div className="excel-guide-footer">
        <div className="guide-title">
          ...
          매출원장 엑셀 데이터 안전 관리 가이드
        </div>
        <ul className="guide-list">
          <li>매출원장 엑셀 업로드 시 기존 매출 내역과 일자, 거래처 등이 겹치게 되면 중복 등록 또는 합계 오류가 발생할 수 있습니다.</li>
          <li>기존 등록된 매출 장부를 대량으로 덮어쓰기 하거나 업로드하기 전에 반드시 '매출원장 다운로드'를 통해 백업해 두세요.</li>
          <li>금액 필드(공급가액, 세액 등)가 텍스트 형식으로 되어 있는 경우, 합산이나 결산 보고서 분석에서 정상 계산이 되지 않을 수 있습니다.</li>
        </ul>
      </div>
    </WindowModal>
  );
...
export default SalesLedgerExcelManager;

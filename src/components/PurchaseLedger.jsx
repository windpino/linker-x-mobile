...
import { BookOpen, Printer, Download, Search, X } from 'lucide-react';
import WindowModal from './WindowModal';
import { matchesInitialSound } from '../utils/koreanUtils';
import { exportToExcel } from '../utils/excelUtils';
import './PurchaseInvoice.css';
import './PurchaseManagementCommon.css';

const PurchaseLedger = ({ onClose, purchaseInvoices = [], partners = [], onOpenInvoice, currentUser, onUpdateInvoice, zIndex, initialPartner = '', initialDate = '' }) => {
  const [colWidths, setColWidths] = useState({
    ...
    supplyValue: 110,
    total: 110,
    received: 110, // paid amount
    balance: 120, // unpaid balance
    creator: 90,
    createdAt: 90,
    updatedAt: 90
  });
  ...
  rows.push({
    ...
    supplyValue: '',
    total: '',
    received: '', // represents paid/outflow
  ...
  sortedDates.forEach(dateStr => {
    ...
    dayInvoices.forEach(inv => {
      ...
      // B. Payment rows (Outflow)
      const paymentsList = [];
      if (inv.payments) {
        if (inv.payments.cash > 0) paymentsList.push({ type: 'cash', amount: inv.payments.cash, label: '출금(현금)', spec: '' });
        if (inv.payments.account > 0) paymentsList.push({ type: 'account', amount: inv.payments.account, label: '출금(통장)', spec: partnerBankAccount && partnerBankAccount !== '선택안함' ? partnerBankAccount : '' });
        if (inv.payments.card > 0) paymentsList.push({ type: 'card', amount: inv.payments.card, label: '출금(카드)', spec: '' });
        if (inv.payments.bill > 0) paymentsList.push({ type: 'bill', amount: inv.payments.bill, label: '출금(어음)', spec: '' });
      }
      if (paymentsList.length === 0 && (inv.paidAmount || 0) > 0) {
        paymentsList.push({ type: 'cash', amount: inv.paidAmount, label: '출금(현금)', spec: '' });
      }

      paymentsList.forEach((pm, pmIdx) => {
        runningBalance -= pm.amount;
        dayTotalPaid += pm.amount;
        ...
        dayRows.push({
          ...
          supplyValue: '',
          total: '',
          received: pm.amount, // output column
          balance: runningBalance,
  ...
  const currentSummaryInvoices = purchaseInvoices.filter(inv => {
    if (inv.date < startDate || inv.date > endDate) return false;
    if (selectedPartner) return inv.partner === selectedPartner;
    return true;
  });
  currentSummaryInvoices.forEach(inv => {
    totalPurchases += inv.items ? inv.items.reduce((s, i) => s + (i.total || 0), 0) : (inv.totalAmount || 0);
    totalPaid += (inv.paidAmount || 0) + (inv.discount || 0);
    totalQty += inv.items ? inv.items.reduce((s, i) => s + Number(i.qty || 0), 0) : 0;
  });
  balance = carriedForward + totalPurchases - totalPaid;
  ...
  const handleExcelExport = () => {
    const dataToExport = rows.map(r => ({
      ...
      '출금액': r.received !== '' && r.received !== undefined ? Number(r.received) : '',
      '잔액': r.balance !== '' && r.balance !== undefined ? Number(r.balance) : '',
      '발행자': r.creator || ''
    }));

    const filename = `매입원장_${selectedPartner || '전체거래처'}`;
    exportToExcel(dataToExport, filename);
  };

  const handleInlineEdit = (invoiceId, itemId, field, value, paymentType = null) => {
    const val = Number(value) || 0;
    const originalInv = purchaseInvoices.find(inv => inv.id === invoiceId);
    ...
    if (field === 'received') {
      ...
      if (paymentType) {
        ...
        updatedInv.paidAmount = (newPayments.cash || 0) + (newPayments.account || 0) + (newPayments.card || 0) + (newPayments.bill || 0);
      } else {
        updatedInv.paidAmount = val;
  ...
  return (
    <WindowModal title="매입원장" onClose={onClose} width="1250px" zIndex={zIndex}>
              ...
              <thead>
                <tr>
                  {[
                    ...
                    { key: 'received', label: '출금액', width: colWidths.received, align: 'right' },
              ...
              <tbody>
                {rows.length === 0 ? (
                  ...
                  rows.map((r, idx) => {
                    ...
                    const inv = r.invoiceId ? purchaseInvoices.find(i => i.id === r.invoiceId) : null;
                    ...
                    return (
                      ...
                      >
                        ...
                        <td className={r.rowType === 'subtotal' ? 'cell-name text-center' : 'cell-name'}>
                          {r.rowType === 'purchase' || r.rowType === 'normal' ? (
                            <span 
                              onClick={(e) => {
                                e.stopPropagation();
                                if (inv) {
                        ...
                        <td className="text-right">
                          {isAdmin && (r.rowType === 'purchase' || r.rowType === 'normal') ? (
                            <input 
                              type="number" 
                              value={r.qty} 
                        ...
                        <td className="text-right font-semibold" style={{ color: r.balance > 0 ? '#ea580c' : '#475569', fontWeight: 700 }}>
      ...
      <style>{`
        ...
        .row-payment .cell-name {
          color: #047857 !important; /* Green-700 for 출금 */
          font-weight: 600;
        }

  ...
  return (
    <WindowModal title={editingOrder ? (isMobileView ? "수주 수정" : "간편수주 수정") : (isMobileView ? "수주 등록" : "간편수주 등록")} onClose={onClose} width="1200px" contentPadding="0">
      <div className="so-wrapper">
        <div className="so-header" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'nowrap' }}>
          <div className="so-title" style={{ display: 'flex', alignItems: 'center', gap: '6px', flexShrink: 0 }}>
            <ShoppingCart size={isMobileView ? 20 : 24} />
            <span className="so-date-badge" style={{ display: 'inline-flex', alignItems: 'center', marginLeft: 0 }}>
              <input 
                type="date"
              ...
              {orderData.partner && (partnerDayOrders.length > 0 || orderData.itemsText) && (
                <span className="so-nav-controls" style={{ marginLeft: '4px', paddingLeft: '4px' }}>
                  <button 
                  ...
                  >◀</button>
                  <span style={{ fontSize: '0.7rem' }}>{currentIndex >= 0 ? currentIndex + 1 : partnerDayOrders.length + 1}/{Math.max(partnerDayOrders.length, currentIndex >= 0 ? currentIndex + 1 : partnerDayOrders.length + 1)}</span>
                  <button 
            ...
            {!isMobileView && (
              <span style={{ fontSize: '0.95rem', fontWeight: 800, color: '#1e293b', marginLeft: '6px' }}>
                {editingOrder ? "간편수주 수정" : "간편수주 등록"}
              </span>
            )}
          </div>
          <div className="so-header-actions" style={{ display: 'flex', gap: '6px' }}>
            ...
            }} style={{ padding: isMobileView ? '6px 10px' : '8px 16px', fontSize: isMobileView ? '0.78rem' : '0.9rem' }}>
              <RefreshCw size={12} /> 새수주
            </button>
            <button className="so-btn-outline" onClick={onOpenOrderList} style={{ padding: isMobileView ? '6px 10px' : '8px 16px', fontSize: isMobileView ? '0.78rem' : '0.9rem' }}>
              <List size={12} /> 목록
            </button>
          </div>
        </div>
            ...
            <div className="so-pane-left">
              <div className="so-card">
                <h3 className="so-card-title" 
                    ...
                    style={{ cursor: 'pointer', display: 'flex', justifyContent: 'space-between', width: '100%', userSelect: 'none' }}>
                  <span style={{ display: 'flex', alignItems: 'center', gap: '8px' }}><Info size={16}/> 상세 설정 (담당자/창고)</span>

...
const companies = ['DMK', 'jinmi', 'windpino_ldmo', 'default'];
const collections = [
...
async function run() {
  for (const companyId of companies) {
    ...
    // Inventory setting check
    const invDoc = doc(db, 'companies', companyId, 'settings', 'inventory');
    const invSnap = await getDoc(invDoc);
    if (invSnap.exists()) {
      console.log(`Inventory doc: exists, value keys:`, Object.keys(invSnap.data().value || {}));
      console.log(`Inventory doc data:`, JSON.stringify(invSnap.data()));
    } else {
      console.log(`Inventory doc: NOT exists`);
    }
